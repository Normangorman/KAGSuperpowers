...
void onTick(CBlob@ this)
{
	         ...
	         (pressed_a2 || (specialShieldState)))
	{
		...
		else
		{
			bool forcedrop = (vel.y > Maths::Max(Maths::Abs(vel.x), 2.0f) &&
			                  moveVars.fallCount > getGlideDownTime(this));
			...
			if (knight.state == KnightStates::shieldgliding && !this.isInWater() && !forcedrop)
			{
				...
				f32 glide_amount = 1.0f - (moveVars.fallCount / f32(getGlideDownTime(this) * 2));

				if (vel.y > -1.0f)
				{
                    float feather_factor = 1.0;
                    if (hasPower(this, Powers::FEATHER))
                        feather_factor = 0.61;
					this.AddForce(Vec2f(0, -20.0f * glide_amount * feather_factor));
				}
	...
	{
		if (getNet().isClient() && knight.state == KnightStates::sword_drawn)
		{
			if (knight.swordTimer == getSlashChargeLevel2(this))
			{
				Sound::Play("AnimeSword.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
			else if (knight.swordTimer == getSlashCharge(this))
			{
		...
		bool strong = (knight.swordTimer > getSlashChargeLevel2(this));
		...
		if (knight.state == KnightStates::sword_drawn && !pressed_a1 &&
		        !this.isKeyJustReleased(key_action1) && delta > getResheathTime(this))
		{
			knight.state = KnightStates::normal;
		}
		else if (this.isKeyJustReleased(key_action1) && knight.state == KnightStates::sword_drawn)
		{
			...
			else if (delta < getSlashChargeLevel2(this))
			{
		         ...
		         knight.state <= KnightStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < getDeltaEndAttack(this))
			{
				f32 attackarc = 90.0f;
				...
				DoAttack(this, getJabDamage(this), attackAngle, attackarc, Hitters::sword, delta, knight);
			}
			else if (delta >= 9)
			{
				knight.swordTimer = 0;
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			{
				DoAttack(this, getSlashDamage(this), -(vec.Angle()), 120.0f, Hitters::sword, delta, knight);
			}
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
				{
					...
					if (ground || wood || dirt_stone || gold || hasPower(this, Powers::STRENGTH) && castle)
					{
						...
						if (dif < 20.0f)
						{
							...
							if (canhit)
							{
                                float dmg = hasPower(this, Powers::STRENGTH) ? 0.3f : 0.1f;
								map.server_DestroyTile(hi.hitpos, dmg, this);
							}
						}
					}
				}
...
float getJabDamage(CBlob@ this) {
    if (hasPower(this, Powers::STRENGTH)) {
        return 1.5f;
    }
    else {
        return 1.0f;
...
float getSlashDamage(CBlob@ this) {
    if (hasPower(this, Powers::STRENGTH)) {
        return 3.0f;
    }
    else {
...
s32 getResheathTime(CBlob@ this) {
    return 1;
...
s32 getGlideDownTime(CBlob@ this) {
    if (hasPower(this, Powers::FEATHER)) {
        return 100;
    }
    else {
...
int getDeltaEndAttack(CBlob@ this) {
    if (hasPower(this, Powers::QUICK_ATTACK)) {
        return 2;
    }
    else {
