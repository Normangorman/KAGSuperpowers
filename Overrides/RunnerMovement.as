...
void onTick(CMovement@ this)
{
    ...
    if (hasPower(blob, Powers::FEATHER)) {
        shape.SetGravityScale(blob.get_f32("gravity scale"));
    }
    else {
	...
	if (moveVars.jumpFactor > 0.01f && !isknocked)
	{
        ...
        else if (hasPower(blob, Powers::TRIPLE_JUMP) &&
                 blob.isKeyJustPressed(key_up) &&
                 blob.get_s8("triple jump count") > 0)
		{
			moveVars.jumpCount = 0;
            tripleJump = true;
		}
		...
		if (up && vel.y > -moveVars.jumpMaxVel)
		{
			...
			if (moveVars.jumpCount <= 0)
			{
                ...
                else if (hasPower(blob, Powers::BOUNCE)) {
                    force.y -= 1.5f * JUMP_MULTIPLIER;
                }
                else {
                    force.y -= 1.5f;
                }
